...
> **Canonical Title**: L04-01 — Why do two loops doing the same logical work run at different speeds? (Memory hierarchy, cache concept, row-major vs column-major traversal intuition)
> **Canonical Module Entry**: Hard `M03` (Machine, ISA & Execution)
> **Canonical Lesson Predecessors**: Hard `L03-03` (Control Flow, ISA & System Execution); Soft `L02-01`
> **Canonical Objectives**:
> 1. 建立非单一延迟的层次化内存心智模型（Memory Hierarchy）；
> 2. 掌握核心概念 `EC-CON-011 Caching` 规范定义与应用边界；
> 3. 理解二维数组在行主序（Row-Major）连续内存下的步长差异对访问复用机会的影响；
> 4. 辨析缓存与持久化存储的本质区别，拒绝将单次运行加速直接当成硬件因果证明。
> **Canonical Primary Competencies**: `Observe`, `Explain`, `Judge`
> **Canonical Concept First Homes**: `EC-CON-011 Caching`
...
| :--- | :--- | :--- | :--- | :--- |
| **Hennessy & Patterson**, *Computer Architecture: A Quantitative Approach* | 6th Ed. (Classic) | STABLE (Checked 2026-09) | Memory hierarchy principle, caching trade-offs, access latency differences | 不规定具体 CPU 型号的精确 L1/L2 周期数 |
| **Row-Major / C11 Array Layout** | ISO/IEC 9899:2011 (C11) | STABLE (Checked 2026-09) | 多维数组在内存中按行主序连续排布，行内相邻元素物理相邻 | 不保证编译器完全不重组循环，需通过实验控制验证 |

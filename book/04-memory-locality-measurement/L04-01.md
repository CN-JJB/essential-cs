...
**Caching（缓存）**：在某种策略与假设下，把数据的副本保留在离消费者更近、通常访问代价更低的位置，以便后续访问有机会复用。缓存是性能机制，不是持久性或权威存储的同义词。它带来 capacity、placement/reuse opportunity、complexity 与 cost 的 Trade-off。

...
> **Canonical Module**: M04 — Memory Hierarchy, Locality & Measurement Uncertainty
> **Canonical Title**: L04-02 — Why does access order matter, and does being faster once prove why? (Locality first home, fair measurement pattern, repeated trials, median/IQR, inference limits)
> **Canonical Module Entry**: Hard `M03`
> **Canonical Lesson Predecessors**: Hard `L04-01` (Memory hierarchy & caching intuition); Soft `L02-02`
> **Canonical Objectives**:
> 1. 掌握核心概念 `EC-CON-012 Locality` 规范第一原籍定义，严格区分时间局部性与空间局部性；
> 2. 掌握应用度量不确定性工具集（Applied Measurement-Uncertainty Pattern）：假设→基线→受控变量→预热→重复试验→中位数/IQR→推断边界；
> 3. 区分实验观察现象与因果假说，给出至少两项竞争性解释（Competing Explanations）；
> 4. 树立严格的推断边界：拒绝微基准测试（Microbenchmark）向生产全场景的无条件外推。
> **Canonical Primary Competencies**: `Observe`, `Diagnose`, `Estimate`, `Explain`, `Judge`
> **Canonical Concept First Homes**: `EC-CON-012 Locality`

## Locality / 局部性 — EC-CON-012

**Locality（局部性）**：相关访问或通信倾向于在时间、空间或放置位置上彼此接近，从而有机会降低资源成本。在本课的硬件场景中，我们重点观察相邻地址访问形成的 **spatial locality（空间局部性）**，并用近期数据再次被访问来理解 **temporal locality（时间局部性）**。Locality 描述访问/放置模式；Caching 是利用某些 locality 机会的机制。两者不是同一个概念。
...
- 复杂的统计学显著性检验（如 Student's t-test、ANOVA 或正态性分布假设）；
- 硬件性能计数器（Performance Counter）底层寄存器微架构配置；
- 高度精细的高阶缓存行失效（Cache Line Invalidation）总线嗅探协议与 MESI 状态机；
- 操作系统虚存超级大页（Transparent Huge Pages）与 NUMA 跨节点内存控制策略。
...
## Provenance / Time-Sensitivity

| Source / Standard | Version / Status | Check Date | Claim Used Here | What It Does Not Prove |
| :--- | :--- | :--- | :--- | :--- |
| **Peter J. Denning**, *The Locality Principle* | CACM 2005 / Foundational | STABLE (Checked 2026-09) | 局部性原理（Locality Principle）：时间局部性与空间局部性的形式化定义 | 不保证所有程序都具有高局部性 |
| **Georges et al.**, *Statistically Rigorous Java Performance Evaluation* | OOPSLA 2007 | STABLE (Checked 2026-09) | 实验公平性、预热与多次重复测量的测量方法学纪律 | 不要求初学者在 M04 进行完整高阶置信区间建模 |
| **Linux / GCC Toolchain Benchmark Stance** | GCC 13/14, Linux 6.x | CURRENT (Checked 2026-09) | `-O2 -fno-tree-vectorize -fno-unroll-loops` 抑制循环向量化混杂 | 不保证排除所有微架构分支预测与调度抖动 |

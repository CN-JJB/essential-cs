...
- 阐述按需调页（Demand Paging）机制：操作系统为何在地址空间预留（Reservation）阶段仅建立虚拟映射描述符，直到代码发生首次写操作（First Touch）产生次要缺页异常（Minor Page Fault）时才真正分配并绑定物理页帧；
...
flowchart TB
    subgraph Phase1["第 1 阶段：地址空间预留（Reservation）"]
        ...
        VMA["内核在进程虚拟空间中登记 VMA 区间\nVmSize (虚拟大小) 显著增加\n此时不分配物理页帧, 页表项尚未绑定物理帧\nVmRSS (物理驻留) 几乎不变"]
    end
    ...
    subgraph Phase2["第 2 阶段：物理帧按需提交（Commit / First Touch）"]
        ...
        BIND["内核分配一个干净的物理页帧 (PFN)\n将物理页填零并写入页表项 (PTE)\nLinux 记入一次次要缺页 (Minor Fault)\nVmRSS 实际物理驻留量增加"]
    end
...
> 我们必须在这里纠正这种教条化认知：
> 1. **分配器具有多样性**：运行时并非所有分配都走 `brk`。glibc 的 `malloc` 对大块内存（如超过 128 KiB）通常直接调用 `mmap`；现代分配器（如 jemalloc、mimalloc）大量使用匿名内存映射与多线程局部缓存池；
...
1. **阶段 0 $\rightarrow$ 阶段 1（仅预留）**：
   - `VmSize` 从 $18,640\text{ KiB}$ 增加到 $35,024\text{ KiB}$，净增恰好为 $16,384\text{ KiB} = 16\text{ MiB}$！虚拟空间已成功登记；
...
2. **阶段 1 $\rightarrow$ 阶段 2（写入前 2048 个页面）**：
   ...
   - `VmRSS` 跃升至 $21,484\text{ KiB}$，增量为 $21,484 - 13,292 = 8,192\text{ KiB} = 8\text{ MiB}$，恰好是 2048 个物理页帧的容量；
   - `Minor Faults` 跃升至 $4,223$ 次，增量为 $4,223 - 2,175 = 2,048$ 次！每一次跨页首次写入，都触发了一次硬件缺页并由 Linux 内核分配了一个干净页帧。
...
这个实验用无可辩驳的真实数字证明：**虚拟内存的承诺是廉价的支票，只有在兑现（写入）时才真正消耗物理金库（RAM）**。
...
- `2`（严格禁止）：绝不过量分配。系统提交的内存总量不能超过 `Swap + RAM * overcommit_ratio`。
...
- 根据进程的内存消耗、特权级、以及 `/proc/<pid>/oom_score` 计算出一个牺牲者得分；
- 挑出得分最高的进程，直接向其发送不可捕获的强制终止信号（通常是 `SIGKILL`，信号 9），将其物理内存全部强行收回，保全整个操作系统的运转。
...
flowchart TD
    ...
    OOM_KILL["内核唤醒 OOM Killer\n按 oom_score 选出牺牲进程\n发送 SIGKILL 强行终止进程以自保"]
...
但小张必须高度警惕潜在风险：如果该服务在后续业务中突然开始全量写入这 8 GiB 空间，需求分页将逼迫内核分配物理帧；在写入量超过 4 GiB 物理可用量时，系统将陷入剧烈的内存颠簸（Paging/Thrashing）甚至触发 OOM Killer 杀死进程。
</details>

<details>
<summary>Full Explanation</summary>
`VmSize` 仅代表操作系统在虚拟地址空间中为该进程保留的范围，由 `mmap` 或分配器预留引起，几乎不消耗物理内存条。当前 `VmRSS` 只有 200 MiB，说明绝大部分虚拟页面从未被写入，处于尚未提交状态。因此机器当前处于健康安全状态。
然而，`VmSize` 反映了该进程理论上的峰值物理需求。一旦程序开始遍历写入这 8 GiB 空间，CPU 将为每个新页面触发缺页异常（Minor Page Fault）。由于物理内存上限仅有 4 GiB，内核将先尝试换出其他页面或回收缓存；若内存依然不足，系统将触发 OOM Killer。
</details>
...
- **“只要代码里调用了 malloc(1GB)，机器的物理内存就立刻少了 1GB。”** 错。在 Linux 等支持按需调页的现代系统上，单纯的分配仅在进程虚拟空间登记范围（增加 `VmSize`），只有真正向页面写入数据时，物理内存（`VmRSS`）才会被实际扣减。
- **“虚拟内存大小（VmSize / VIRT）就是程序当前的内存泄漏量。”** 错。很多现代运行时（如 Go、Java、多线程分配器）为了避免频繁系统调用，在初始化时会预先保留极大范围的虚拟地址跨度。判断内存泄露的核心参考依据是持续增长的**实际驻留物理内存（`VmRSS`）**，而非静态的虚拟预留量。
...
能够清晰解释 `VmSize` 与 `VmRSS` 的区别；用白板画出“分配预留 $\rightarrow$ 首次写入 $\rightarrow$ 缺页异常 $\rightarrow$ 物理帧绑定”的需求分页生命周期；结合 `residency_fixture.py` 的实验数据说明为什么预留阶段不增 RSS，而写入阶段 RSS 与缺页次数成正比增加；准确表述 Overcommit 与 OOM Killer 的成因与限制。

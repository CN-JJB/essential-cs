...
flowchart TD
    ...
    subgraph Layer3["3. 操作系统内核处理层 (Linux Kernel)"]
        HANDLER["内核缺页处理程序 (do_page_fault)\n检查当前进程的 VMA 链表/红黑树"]
        DECIDE{"地址是否落在合法的 VMA 内,\n且操作符合权限?"}
        RESOLVE["良性缺页 (OS 修复后透明重试):\n- 需求分页分配新物理页帧 (Minor Fault)\n- 换入被置换到磁盘的页面 (Major Fault)\n- 写时复制 (COW) 页面分裂\n-> 修复页表, 重新执行原指令"]
...
2. **操作系统决断层（OS Resolution）**：
   ...
   - **可修复（Resolved）**：如果地址落在合法分配区内（例如需求分页的首次写入、写时复制 COW、或者被换出到 Swap 的页面），内核默默分配物理帧或从外存读入数据，填好页表，然后**命令 CPU 重新执行刚刚那条发生异常的指令**。整个过程对用户程序完全透明，程序根本不知道发生过缺页！
   - **不可修复（Unresolved / Invalid）**：如果该地址根本没有在任何 VMA 中登记（如经典的 `0x0`），或者试图向只读代码段（`r-xp`）写入数据，内核断定这是非法操作，无法修复。
3. **Linux 统计学会计分类（Linux Accounting: Minor vs Major）**：
   在 Linux 性能统计中，针对**成功修复的良性缺页**，内核根据是否发生实际磁盘 I/O 划分为：
   - **Minor Fault（次要缺页）**：内存中已有空闲页帧，无需等待磁盘慢速 I/O（如需求分页 zero-fill、COW）；
   - **Major Fault（主要缺页）**：页面已被置换到磁盘 Swap 分区或来自磁盘文件映射，内核必须等待磁盘 I/O 读入物理页。
...
1. **Python Subprocess 负数状态码（`-11`）**：
   ...
   在这里，`-11` 极其精准地证明了子进程并非自主退出，而是被操作系统内核直接发送了 **11 号信号（`SIGSEGV`，段错误）** 强行击毙。
...
3. **系统层**：Linux 内核捕获 #PF 异常后，通过进程的 VMA 列表查找 0x0，确认该地址不属于任何合法的用户内存段。由于无法通过需求分页修复，内核才向该进程的 PCB 发送 POSIX 信号 SIGSEGV。因此，SIGSEGV 是特定宿主环境（Linux / POSIX）下对该 UB 的具体处理结果，而非 C 语言标准的普适保证。
</details>

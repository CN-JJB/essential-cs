...
2. 建立确定性 baseline；
3. 检查一个 boundary；
4. 用 debugger-light 方式在一个真实函数入口停一次并观察传入对象；
5. **只做一个**可逆 controlled change；
6. 保存 change/diff；
7. 重跑；
8. 把 Observation 与 Explanation 分开；
9. 写一个 competing explanation 或 unresolved uncertainty；
10. reset；
11. 为一个 claim 选择正确 evidence/source layer。
...
## 贯穿项目路标：Mini Cloud 起步 (P0)

在整个 Essential CS 学习周期中，除各模块独立机制实验外，你将伴随一个贯穿始终的综合集成项目——**Mini Cloud**（详见 [`project/README.md`](../../project/README.md)）。它是一个刻意保持简约的多用户笔记/书签服务，存在的唯一目的是让你在掌握各项底层机制时，反复追踪**同一个**业务请求在数据表示、进程边界、持久化存储、并发控制、真实网络依赖、故障恢复与可观测性中的真实因果。

- **阶段路标 P0（单进程与持久化集合）**：
  - 核心实现：[`project/minicloud/store.py`](../../project/minicloud/store.py) 与 [`project/minicloud/cli.py`](../../project/minicloud/cli.py)。
  - 核心不变式：每个条目拥有确定归属；记录 ID 唯一且稳定；写后读（read-after-write）严格返回已持久化提交的状态。
  - 契约说明（F-06-08）：P0 可见的最小有序迁移机制即 P6 的同一有界基础设施的早期形态——只保证“一次一个版本、可重放、可回滚到基线”，不引入迁移框架；“每里程碑一个约束”的教学目标不受影响。
  - 快速验证：
    ```bash
    bash project/scripts/preflight.sh
    export PYTHONPATH=project
    python3 -m minicloud.cli init --fixture
    ```
...
- 一次 bounded debugger-light function-entry observation；
...
- AI boundary adheres to D-033 (resolving OQ-BP-001 for v1.0 without late Core expansion; AI outputs treated as unverified candidate hypotheses requiring source, test, measurement, and security verification).

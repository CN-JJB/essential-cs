...
你会看到同样的两 bytes 在 wrong byte-order rule 下被解成另一个 value。

这说明 round trip 依赖 encoder/decoder 对 representation contract 的一致理解。
...
truncated UTF-8 无法按完整规则 decode。

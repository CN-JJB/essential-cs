...
真实工程很少有“永远最快”的结构。一个选择可能降低 lookup growth，却增加 build、memory、maintenance 或 correctness obligation。只背“hash 是 `O(1)`”会把真正的工程判断隐藏掉。
...
- data structure 简单；
- 没有额外 index build；
- 没有 index 与 records 同步的额外 invariant；
...
| Correctness obligation | unique-key rule still matters | unique-key + index synchronization invariant |
...
| When NOT to use | when tiny/short-lived, memory constrained, rebuild-heavy, or lookup not dominant |

**图/表的教学目的：**术中同一个 `lookup(key)` Interface 可以由不同 Representation/State 组织实作；更住的 lookup growth 不是六空出现，它从成本移到 build，memory，maintenance 与 correctness obligation。

## Predict，同一 darget，先写 count
...
LOOKUPnote=indexed_count is a logical interface probe, not free machine work
```

这亚列不能直場当同一种机器服 operation 的须全成本；linear 列数的是 key comparisons；index build 数 logical insertions；indexed lookup 数 interface-level logical probes。
...
> 在“collection 建好后会执行许多 exact-key lookup、memory 可接受、mutation 很少”的约束下，维护 key-based index 可以把 lookup growth 从 linear scan 的 `O(n)` 降到 average-case `O(1)` model；代价是一次 `O(n)` build、额外 memory、mutation maintenance，以及必须保持 index/records 一致的新 correctness obligation。
...
Trade-off 要把 gain 和 cost 放在同一个约束下。key-based index 在 repeated exact-key lookup 上有更好的 growth，但它不是 free：要先构建、占额外 memory、维护 derived state，并承担新的 correctness obligation。对 `n=8` 且只 lookup 一次的短生命周期 workload，这些成本可能没有回报。asymptotic notation 仍然正确地描述增长，但不足以单独决定每一个 concrete measured-time choice。
...
- **Correctness：**识别额外 index 会引入 synchronization obligation；正式 Specification/Invariant/Correctness 在 L02-03 首次定义。

...
Specification
  ↓ 规定外部需要什么行为
Implementation + permitted transitions
  ↓ 每次 transition 后检查
Invariant
  ↓
Correctness claim under stated assumptions
```
...
> 在指定 model 下，经过每一个 permitted state transition 后都必须保持为真的 property。

它不是“通常是真的”。如果一个允许的 transition 能让它变成 false，那么 invariant 已经被破坏。

### Correctness / 正确性

**Correctness** 是：

> implementation 的 observable behavior 在 stated assumptions 下满足 relevant Specification，并保持所需 Invariant。

Correctness 不是 speed、安全性或“某个 sample 跑成功”的同义词。

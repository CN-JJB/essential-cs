...
## Evidence Boundary｜你知道什么，哪些只是当前实现观测？

在填写证据记录时，请严格约束语言：

- **错误断言**：“CPython 只是把 Python 源码逐行直接交给 CPU。”（错；对本课观察的代码单元，当前 CPython 会先形成自己的内部可执行表示，再由运行时执行。代码单元边界随入口而异，不要扩成‘所有 Python 程序都一次性编译整个文件’）。
- **错误断言**：“`LOAD_FAST_LOAD_FAST` 是 Python 加法的标准指令。”（错；它是作者/Lead 在当前 CPython 3.13.x fixture 中观察到的实现细节，其他 Python VM 或 CPython 版本不应假定相同）。
- **精准表述**：“在我记录的当前 CPython 实现/版本中，这个 fixture 的字节码包含参数加载、二元运算与返回等关系；这些字节码由 CPython 运行时执行，而具体 opcode 名称、内部函数与宿主机器指令并不是 Python 语言规范保证。”

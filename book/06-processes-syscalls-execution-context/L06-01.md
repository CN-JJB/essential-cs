...
## 必修核心实验路由：LAB-REQ-02 — xv6 系统调用链路全景穿透

- **何时执行**：在理解了 CPU 特权模式切换与系统调用基本概念后，应执行本模块的正式必修实验 `LAB-REQ-02`。
- **与基础活动关系**：前述 `labs/foundations/m06/process_observer.py` 是宿主机（Linux）上审查 `/proc/self` 与 PID 的轻量级观察脚本；而 `LAB-REQ-02`（位于 `labs/lab-req-02-xv6-syscall/`）是 M06 的正式考核必修实验（Assessed Required Lab）。它在真实开源操作系统源码（MIT 6.1810 xv6 RISC-V）中追踪从用户态命令到硬件 trap 指令（`ecall`）再到内核分发函数的完整路径。
- **前置准备与预检**：
  ```bash
  cd labs/lab-req-02-xv6-syscall
  ./preflight.sh     # 检查 QEMU 与 RISC-V 交叉编译工具链
  ./setup.sh         # 初始化并校验 pinned xv6 worktree 源码
  python3 verify_source_route.py worktree # 静态验证系统调用源码锚点
  ```
- **核心实现与运行**：
  编写 `worktree/user/sleep.c` 并将其加入 `worktree/Makefile` 的 `UPROGS` 列表，随后执行：
  ```bash
  ./smoke.sh
  ```
- **达标证据与退出准则**：
  `smoke.sh` 运行成功，在 QEMU 控制台中正确输出 `Usage: sleep ticks`，运行 `sleep 10` 正常返回，并打印出唯一的 `LAB_REQ_02_OK` 标识行；所有子进程干净回收。学习者在 `course/evidence/lab-req-02-xv6-syscall-evidence-template.md` 记录反汇编关系与实际运行数据。实验后可运行 `./reset.sh` 恢复工作区。
...
- **Learn-New-Tech**：自主阅读并理解真实开源操作系统（MIT xv6 RISC-V）的系统调用派发实现与用户态工具开发。

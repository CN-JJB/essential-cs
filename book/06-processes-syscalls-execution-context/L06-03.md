...
## 贯穿项目路标：Mini Cloud 进程边界与服务启动 (P1)

在掌握进程上下文与状态模型之后，可在贯穿项目 Mini Cloud 中观察进程隔离与服务边界：

- **阶段路标 P1（进程边界与窄接口）**：
  - 核心实现：[`project/minicloud/httpd.py`](../../project/minicloud/httpd.py)、[`project/minicloud/client.py`](../../project/minicloud/client.py) 与 [`project/minicloud/cli.py`](../../project/minicloud/cli.py)。
  - 机制映射：服务端作为一个独立的操作系统进程运行（`python3 -m minicloud.cli serve`），通过操作系统套接字（Socket）与客户端进程（`curl` 或测试脚本）交互。这一进程边界构成了第一道防御隔离：外部未校验的输入必须在进程网络入口完成窄接口校验并返回确定性的有界错误，严禁穿透边界破坏底层执行上下文。
  - 契约说明（F-06-04）：此处先立“窄接口优先”的进程边界事实；HTTP 适配器在本阶段只是边界载体，其完整 HTTP 语义（方法/状态/缓存/代理）留待 M11–M12 回访，不把网络课程提前。
  - 验证命令：
    ```bash
    export PYTHONPATH=project
    python3 -m minicloud.cli serve --port 8765 &
    SERVER_PID=$!
    ps -p $SERVER_PID -o pid,state,cmd

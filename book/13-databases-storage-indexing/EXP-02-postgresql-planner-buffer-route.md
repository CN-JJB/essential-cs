...
- **主要能力**：**Observe** (在生产级企业关系型数据库源码中观测查询代价模型与缓冲池系统实现)
- **进阶能力**：**Trace** (追踪从代价估算公式到缓冲页置换策略的逻辑脉络), **Explain** (阐明工业级实现与教学概念模型的异同)
- **代码仓库**：[PostgreSQL 官方 Git 源码树](https://git.postgresql.org/gitweb/?p=postgresql.git) / [GitHub 镜像](https://github.com/postgres/postgres)
- **考察方式**：**在线链接与代码导读（Link-and-Inspection-First）**。严禁完整克隆庞大仓库，严禁本地编译 PostgreSQL。

---

## 1. 考察背景与教育目标

在课堂与教材中，我们学习了数据库如何根据物理访问路径选择全表扫描（Sequential Scan）或索引扫描（Index Scan），以及存储引擎如何通过缓冲池（Buffer Pool）在内存中缓存磁盘数据页。这些机制在最具代表性的生产级开源关系型数据库 PostgreSQL 中究竟是如何落地的？

PostgreSQL 经过数十年的工程演进，拥有极其稳固的系统架构。本考察通过**严格受控的三锚点路径（Three-Anchor Route）**，带领学习者直击 PostgreSQL 内部两处最关键的核心机制：
...
1. [计划生成文档锚点]
   ...
   * Target 1 关键警示: 该文档主要聚焦于子查询规划的历史细节，而非通用的 Path->Plan 宏观架构
   * 停止点: 读完文档开篇与概述即止; 严禁追踪复杂的递归子查询展开逻辑!
         |
         v
2. [物理访问路径代价估算]
   "优化器如何量化全表扫描与索引扫描的成本?"
...
3. [存储引擎缓冲池管理]
   ...
   * 停止点: 停在 Clock Sweep 算法描述处; 严禁下潜到 lwlock 底层实现与 WAL 恢复重放!
========================================================================================================
```
...
> - **严禁编译 PostgreSQL**；
> - **严禁漫游泛化调用链**：到达指定函数入口与核心公式后立即停止，绝不顺着头文件无限制深挖；
> - **严禁代码整段搬运**：本考察以官方链接与学习者自己的总结释义为主。遵循 PostgreSQL License，不得无节制复制代码。
...
- **Target 1 关键警示（Historical Caveat）**：
  在本次检查的 PostgreSQL 源码版本中，`src/backend/optimizer/plan/README` **并不是一份通用的“Path 到 Plan 整体转换”的系统架构综述，而是一份聚焦于子查询与子计划（Subselect / Subplan Planning）处理历史与特定算法演进的文档**。
  *(架构审查补充注：若需查阅优化器宏观阶段综述，可参考上级目录的 `src/backend/optimizer/README` 作为审阅背景，但本考察的受限学习路径严格以 `plan/README` 为实证实测锚点。)*
- **考察问题**：
  该文档开篇阐述了什么核心主题？它如何描述计划树（Plan Tree）对于子查询的规划开销？
- **实证记录**：
...
- **考察问题**：
  1. 在 `cost_seqscan()` 中，PostgreSQL 是如何结合磁盘页面数（`baserel->pages`）与元组数（`baserel->tuples`）计算启动代价（`startup_cost`）与总代价（`total_cost`）的？涉及了哪些具名系统参数（如 `seq_page_cost`、`cpu_tuple_cost`）？
  2. 在 `cost_index()` 中，为什么要区分索引树自身的遍历代价与回表访问数据页的代价？为什么 `random_page_cost` 的默认设定值通常高于 `seq_page_cost`？
- **实证记录**：
  1. `cost_seqscan` 核心公式将 I/O 代价与 CPU 代价相加：总代价 $\approx \text{pages} \times \text{seq\_page\_cost} + \text{tuples} \times \text{cpu\_tuple\_cost}$（外加操作符过滤评估代价）。启动代价极低（通常为 0），因为顺序扫描无需提前构建中间树结构。
  2. `cost_index` 计算了遍历索引页面的 I/O、评估索引元组的 CPU 代价，以及根据选择率估算回表读取数据页的随机 I/O 代价。当过滤选择率较差时，大量的随机 I/O 导致估算代价骤增，使得优化器转而倾向全表顺序扫描。
- **停止确认**：已在代价变量相加与返回处停止，未跟踪特定硬件平台的并行扫描修正分支。
...
- **实证结论**：
  **代价是一个经验启发式模型，不是物理时间预测器。** 磁盘寻道延迟、操作系统文件系统缓存（Page Cache）、SSD 固态闪存并发读写特性以及硬件内存带宽在不同机器上差异巨大。PostgreSQL 优化器的目的不是准确预测耗时 12.3 毫秒，而是**在多个候选候选执行路径中，正确选出相对更优的那一个**。把优化器代价值当作绝对性能指标在系统工程上是严重的认知偏差。

### 任务 5（实证版本与元数据记录）：源头版本追踪

学习者每次考察都必须自行记录：
- **实际检查时间**：`<your actual timestamp>`
- **官方源码 Host**：`git.postgresql.org` 或 GitHub mirror
- **考察分支/ref**：例如 `master` 或 `REL_16_STABLE`
- **实际检查 Commit / 标签**：`<your exact PostgreSQL commit hash or tag>`
- **源码访问状态**：`LIVE_POSTGRESQL_SOURCE_ACCESSIBLE` 或 `NO LIVE SOURCE RECHECK`

若无法访问官方代码树，不得凭空捏造提交哈希或伪造代码段；此时只能把仓库内 Design/Research 作为 **REFERENCE EVIDENCE ONLY**。
...
2. **抽象与接口边界（EC-CON-005 Interface）**：查询执行器只需要调用存储层接口获取元组，优化器通过代价抽象屏蔽了物理存储布局的微观细节；
3. **缓存机制（EC-CON-011 Caching）**：缓冲池利用引用固定（Pinning）与 Clock Sweep 策略，在有限内存与并发安全性之间达成了高效平衡。
...
- 本考察采取 **Link-and-Inspection-First** 原则：不镜像、不分发完整 PostgreSQL 源码。所有实证均基于官方链接与学习者自己的总结。任何针对具体源码片段的引用均应注明源文件路径与开源版权声明。

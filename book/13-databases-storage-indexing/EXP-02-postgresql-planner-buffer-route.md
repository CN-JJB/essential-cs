...
### 三步 Source Route 完成后的反思（不新增 learner path）：代价与绝对速度

- **反思问题**：
  为什么 PostgreSQL 优化器计算出的 `cost` 是无量纲的“代价值”，而不是具体的“执行毫秒数”？
- **实证结论**：
  **代价是规划器用于比较候选路径的估计模型，不是物理时间预测器。** 磁盘延迟、操作系统缓存（Page Cache）、SSD 行为以及内存状态会随机器与工作负载变化。PostgreSQL 使用这些代价值选择**估计代价较低**的候选方案；统计误差、参数与实际硬件都可能使真实运行表现不同，因此不能把 cost 当成绝对毫秒数，也不能把“最低估计 cost”解释为对真实全局最优的保证。
...
1. **权衡取舍（EC-CON-006 Trade-off）与局部性（EC-CON-012 Locality）**：`costsize.c` 中 `seq_page_cost` 与 `random_page_cost` 编码了规划器对不同页面访问模式的相对成本假设；它们是可配置的代价参数，不是对某台硬件物理 I/O 延迟的直接测量或跨设备常数；

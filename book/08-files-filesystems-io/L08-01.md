...
## 横向线索｜数据残留、元数据暴露与静态数据责任 (Privacy & Data Responsibility)

在掌握了“路径名 $\to$ 目录项 $\to$ Inode $\to$ 存储块”的四层解耦模型后，我们必须从系统责任（Data Responsibility）与隐私边界的角度审视存储生命周期：

1. **`unlink()` 并非物理销毁（Data Remanence 风险）**：
   - **接口真相**：`unlink()` 只是从父目录中删除了一个名字（目录项），并在 `st_nlink == 0` 且所有进程关闭该文件时，将 Inode 和数据块在文件系统位图中标记为“空闲（Free）”。
   - **物理介质现实**：底层磁盘磁道或固态硬盘（SSD）的 Flash 页上，**原始数据比特依然原样存在**，直到未来某次写操作覆写这部分物理块。在带有磨损均衡（Wear Leveling）与垃圾回收（GC）的现代 SSD 上，即便在文件层写入全 0，旧数据也可能在控制器内部保留块中留存。
   - **责任推论**：在没有加密或硬件安全擦除（Secure Erase / TRIM）的系统上，退役磁盘、归还云主机实例或遗失存储介质，极易导致已被逻辑“删除”的敏感个人数据被逆向取证还原。

2. **临时目录、交换分区与元数据泄漏**：
   - 进程在 `/tmp` 或 `/var/tmp` 创建的未加密临时文件，即使程序退出时自动 `unlink()`，若遭遇非正常宕机，数据便会长久留在磁盘上；
   - 当系统内存紧张发生换页时，匿名内存页（包括进程内存中的明文密钥、用户 Session Token、个人身份信息 PII）会被写入磁盘上的交换分区（Swap Space）。若交换分区未加密，攻击者即可通过离线读取 Swap 镜像窃取敏感数据；
   - **元数据暴露**：即使文件内容被加密，文件名（如 `patient_10023_report.pdf`）、文件大小、创建/修改时间戳（mtime/ctime）以及所有者 UID/GID 也会直接暴露用户的行为规律与敏感特征。

3. **架构防御原则：数据最小化与加密擦除（Crypto-shredding）**：
   - **数据最小化（Data Minimization）**：不写入非必要的明文调试日志或中间临时文件；临时敏感数据应优先利用 `O_TMPFILE`、`memfd_create()` 或 `tmpfs`（仅存在于内存的虚拟文件系统），避免不经意落入物理持久介质；
   - **加密擦除（Crypto-shredding）**：在应用层或文件系统层为每个租户或敏感对象使用独立的数据加密密钥（DEK）。当需要“彻底销毁”用户数据时，只要安全销毁对应的加密密钥，即可使物理介质上残留的所有历史密文块在数学上不可逆解密，从根本上解决物理擦除不可靠的困境。

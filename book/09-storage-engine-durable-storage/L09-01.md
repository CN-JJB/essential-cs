...
- **Diagnose**：解释为什么在 Linux 上仅同步文件 fd 并不必然同步目录项，并能根据新建/同目录 rename/跨目录 rename 识别需要纳入同步协议的相关目录 fd；
...
### 关键数据与推导边界：
1. **宿主观测**：在当前测试环境下，纯缓冲写入 50 条小记录均值约 0.32 毫秒，而每写一条记录就调用一次 `fsync` 的该组试验均值约 240 毫秒。`fsync` 会阻塞调用线程直到同步动作完成或报错，但 CPU 可以调度其它线程/进程；不能把这次时延归结为一个跨系统固定的“CPU 停顿 + 驱动落盘”公式；
2. **严禁将比例当定律**：不同 OS、文件系统、虚拟化层、设备缓存策略和 workload 会产生不同同步时延分布。**这里唯一可推广的是“记录原始样本并按宿主证据解释”，而不是任何固定微秒/毫秒或倍率。**
...
该声明缺少必要的同步契约。普通 append 成功本身不足以证明 WAL 已进入具名掉电模型要求的稳定日志存储；如果故障发生在日志尚未满足该同步契约之前，相关日志**可能无法在重启后恢复**，因此不能向用户承诺“100% 不丢”。
WAL 无法凭空创造持久性。Group Commit 是一种常见优化方向：多个事务共享一次日志同步完成点，从而摊薄同步成本；具体批大小、等待策略与 failure model 由存储引擎设计决定。
</details>
...
普通 buffered 写日志后，记录可能仍处于运行时/OS/文件系统的易失状态；是否已进入稳定日志存储不能由 `write()` 返回值单独判断。在 sudden-power-loss 模型下，如果该日志没有满足系统规定的同步协议，它**可能不在恢复时可见**。因此“不满足日志同步契约却仍承诺掉电不丢”没有足够证据。

<strong>2. 工业级兼顾方案：组提交（Group Commit）</strong>
逐事务单独同步可能带来较高延迟/吞吐成本，但幅度取决于设备与 workload。许多数据库使用组提交类机制来摊薄日志同步成本：
1. 允许并发事务将各自的日志条目先追加到共享的日志缓冲区中；
2. 当第一个事务准备执行 <code>fsync</code> 时，后续到达的若干并发事务并不各自发起独立的系统调用，而是挂起等待；
3. 协调者发起一次日志同步，使该同步完成点覆盖当前批次中已经进入日志的若干事务；批次规模不是固定“十几个/上百个”；
4. <code>fsync</code> 完成后，批量唤醒所有参与组提交的事务，并向所有客户端返回成功。
通过 batching + amortized sync，可以在**仍满足该引擎日志同步/failure-model 契约**的前提下减少每事务独立同步的开销；它并不自动升级为“任意掉电模型下绝对持久”。
</details>
...
- **“只要 `write()` 成功返回了写入字节数，数据就是掉电持久的。”** 错。成功返回不等于已经满足具名 power-loss failure model 的同步要求。
- **“`close()` 可以当作 `fsync()` 的等价持久性证据。”** 错。语言运行时关闭文件对象可能先处理其用户态缓冲，但 POSIX/Linux `close(2)` 成功本身不提供 `fsync()` 等价的同步保证。
- **“只要调用了 `fsync(file_fd)`，新创建的路径名就一定满足掉电持久性。”** 错。Linux 明确区分文件同步与包含它的目录项同步；若 failure model 要求新路径名存活，还需把相关目录同步纳入协议，并处理目录 `fsync()` 的实际结果。
- **“`fsync` 是备份的替代品。”** 错。`fsync` 是文件同步接口；它不解决设备丢失、误删、勒索软件、跨故障域灾难等备份/复制要处理的问题。
- **“有 Replication 就可以不再定义同步/确认规则。”** 错。复制会改变 failure model，但是否需要本地同步、需要多少副本确认、是否要求远端持久确认，都取决于协议要承诺的故障界限；不能从“有副本”推出统一答案。
- **“数据 append 到 WAL 文件就天然持久了。”** 错。事务确认前必须满足该存储引擎为目标 failure model 定义的**日志稳定存储/同步协议**；具体可以通过 `fsync`、同步打开方式或其它受支持机制实现，不能把接口名本身当成全部 durability 证明。

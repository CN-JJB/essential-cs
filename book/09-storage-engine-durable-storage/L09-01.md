...
>
> **边界警示（What it is NOT）**：
> - 持久性 **不等于** “写入了一个文件”（上一模块 M08 已实测证明，普通写入只进入易失性 RAM 页缓存）；
...
工程师在宣称“系统已具备持久性”时，必须立即指明：**它能够抵御哪一层级的故障？**

```mermaid
flowchart TD
    subgraph FailureHierarchy["具名故障与重启界限（Named Failure Hierarchy）"]
        F1["界限 1: 应用程序进程崩溃 (Process Crash)<br>• 应对: 操作系统页缓存兜底，数据未丢"]
        F2["界限 2: 操作系统崩溃 / 内核崩溃 (Kernel Panic)<br>• 应对: 易失 RAM 数据丢失，需已同步至磁盘控制器"]
        F3["界限 3: 服务器干净重启 (Clean Machine Restart)<br>• 应对: 系统关机序列中执行 sync 与 umount"]
        F4["界限 4: 机房意外瞬间掉电 (Sudden Power Loss)<br>• 应对: 必须已写入非易失介质，或设备有掉电保护电容 (PLP)"]
        F5["界限 5: 物理存储介质完全损毁 (Drive Destruction / Disaster)<br>• 应对: 跨机器复制、异地备份、冷归档"]
    end

    F1 --> F2 --> F3 --> F4 --> F5
```

- **场景 A**：一个程序在内存中写完数据，执行了 `f.flush()`。如果此时应用程序发生 `SIGSEGV` 闪退（界限 1：进程崩溃），操作系统依然活着，数据留在页缓存中，最终会被回写。在这个故障界限内，数据是“持久”的；
- **场景 B**：但如果在回写前服务器电源线被踢掉（界限 4：意外掉电），内存断电电荷流失，数据全部灭失。在此故障模型下，刚才的保存操作**不具备持久性**；
- **场景 C**：即使程序对本地 SSD 调用了 `fsync`，成功落入闪存颗粒（抵抗界限 4）。但如果机房发生火灾导致硬盘物理烧毁（界限 5：介质物理损毁），数据依然不存。抵抗界限 5 必须依赖跨故障域的异地冗余备份。
...
数据从代码中的变量流向真正不可动摇的物理介质，需要穿透一系列存在不确定性的缓冲区与检查点：
...
- **`fsync(int fd)`**：向操作系统发出同步请求，要求将属于该文件的所有脏数据页以及**文件元数据（元数据修改时间戳、大小、Inode 属性）**全部刷新至存储设备并等待完成；
- **`fdatasync(int fd)`**：更精细的同步接口。如果某些元数据的变动并不影响随后的数据读取（例如仅仅是文件的最后访问时间 `st_atime` 改变了），`fdatasync` 允许跳过这部分元数据落盘，仅同步数据块内容与必要的文件大小变更，从而减少磁盘磁头寻道或元数据日志刷盘开销。

### 2. 致命盲区：文件数据与父目录元数据的同步断层
许多开发者在代码中这样做：
```python
# 致命错误范例
...
```
**隐患揭秘**：`os.fsync(f.fileno())` 仅仅同步了 `new_order.json` 对应 Inode 的数据块与 Inode 属性。
但是，**将文件名 `"new_order.json"` 映射到该 Inode 的目录项（dentry），存放在父目录 `/var/data` 的数据结构中！**
如果此时突然断电，文件数据块可能确实已写入闪存，但父目录中的目录项尚未落盘。重启后，文件系统找不到这个文件名，该文件就成了无法通过路径访问的孤儿 Inode（甚至在 `fsck` 修复时被当作悬空块清除）。

**严密规范做法**：对于**新建文件**、**重命名（`rename`）**或**删除（`unlink`）**操作，除了同步文件描述符，还必须对**父目录本身**打开一个只读描述符并执行 `fsync`：
```python
# 工业级标准落盘流程
...
# 同步父目录，确保目录项映射入盘
dir_fd = os.open("/var/data", os.O_RDONLY)
os.fsync(dir_fd)
os.close(dir_fd)
```
...
为了兼顾**吞吐量**与**掉电原子一致性**，计算机先辈发明了 **Write-Ahead Logging（WAL，预写日志）**。
...
任何包含修改的脏数据页在被刷入磁盘数据文件之前，**该修改所对应的日志记录必须已经先一步在持久化日志介质中刷盘（`flushed`）！**
如果违反此规则（脏页先落地，日志后落盘或未落盘），一旦在中间断电，系统将无法在重启时判定该脏页修改的来源与旧值，导致数据库彻底破损。
...
2. **重做阶段（Redo）**：从需要恢复的位置起正向重放所有已提交事务的修改，确保即使数据页当时还在内存没来得及写盘，所有已承诺的数据也能 100% 重建回来；
3. **撤销阶段（Undo）**：反向扫描日志，利用旧值（`old_val`）抹去所有未提交事务留下的痕迹，恢复原子性与一致性。

### 3. 边界纠偏：不要神化 WAL
- **WAL 本身并不产生持久性**。如果应用程序仅仅是将数据追加到日志文件尾部（内存页缓存），而没有显式对日志执行 `fsync`，断电时日志本身也会丢失！
- WAL 的真正贡献在于：**它将主数据文件的大范围随机写入（Random I/O），转换为了日志文件的紧凑顺序追加写入（Sequential Append）**。顺序刷盘的物理性能远高于随机刷盘，使得高频事务能够以低成本完成具名持久化。

...
flowchart TD
    ...
    Q3 -- "是" --> Object["候选: 对象存储 (Object Storage)<br>• object/key API<br>• 大规模服务化扩展，受产品配额/成本约束"]
...
2. **Constraints（受限的前提约束）**：
   业务能够接受所选对象服务的实际网络/API 延迟分布，并通过 object/key API 工作，而不依赖 POSIX 原位字节更新、文件锁或本地文件系统延迟假设。
3. **Mechanism（底层核心机制）**：
   对外暴露 object/key + metadata API（常见为 HTTP/HTTPS）；服务端可采用分片、复制、纠删码等多种实现。具体一致性、冗余和布局属于产品契约/实现，不能把“一致性哈希 + 纠删码”当作对象存储的统一定义。
4. **Gains（核心收益）**：
...
6. **Failure Modes（典型失效模式）**：
   - **请求成本放大**：大量小对象/高频 API 调用可能使 request 费用成为重要成本项；是否超过容量费必须按参数计算；
   - **一致性/版本语义误判**：不同服务对覆盖、条件写、版本和列表的契约不同；应用若假设了服务未承诺的语义，会产生竞态或错误。

...
Learner Validation is deferred during authoring but remains mandatory before v1.0 / `RELEASED`. The accepted Research + Design slice for **M00–M04 is fully implemented through M04 after Lead review**. The **M05–M09 Research + Design slice is fully consumed; M05–M09 implementations are Lead-reviewed and merged, including LAB-REQ-02**. The complete **M10–M12 S4 Research + Design slice is Lead-accepted, and M10 implementation is now Lead-reviewed and merged**. The next bounded production step is **M11 learner-facing implementation plus LAB-REQ-01** under the accepted Design, followed by independent Lead review. M12/EXP-03 remains a later bounded implementation batch; do not combine M11 and M12.

## Current priority

1. Implement the bounded **M11 Web Protocols: TLS, HTTP & Caching learner packet plus LAB-REQ-01** from the accepted M10–M12 Design. Keep the Required Lab localhost-only and unprivileged; preserve certificate-path validation vs service-identity boundaries; make `curl` capability a real gate where the accepted Lab requires it; require truthful HTTP/1.1/Via framing and no fabricated external-network/TLS evidence. Do not implement M12/EXP-03 in this batch.

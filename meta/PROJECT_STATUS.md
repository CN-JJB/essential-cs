...
Learner Validation is deferred during authoring but remains mandatory before v1.0 / `RELEASED`. The accepted Research + Design slice for **M00–M04 is fully implemented through M04 after Lead review**. The **M05–M09 Research + Design slice is fully consumed; M05–M09 implementations are Lead-reviewed and merged, including LAB-REQ-02**. The complete **M10–M12 S4 Research + Design slice is Lead-accepted; M10 and M11 implementations are now Lead-reviewed and merged, including LAB-REQ-01**. The next bounded production step is **M12 Web & Browser learner-facing implementation plus EXP-03** under the accepted Design, followed by independent Lead review. Do not reopen M10/M11 scope or combine M12 with later modules.

## Current priority

1. Implement the bounded **M12 Web & Browser learner packet plus EXP-03** from the accepted M10–M12 Design. Keep Web Platform specification claims separate from Chromium implementation/current-practice evidence, require a real browser only for live browser/CORS observations, preserve Site Isolation/browser-process evidence as version/platform-sensitive, and keep M12 event-loop material a preview rather than the EC-CON-015 first home. Do not add M13+ content in this batch.

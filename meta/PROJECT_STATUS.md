...
Learner Validation is deferred during authoring but remains mandatory before v1.0 / `RELEASED`. The accepted Research + Design slice for **M00–M04 is now fully implemented through M04 after Lead review**. The **M05–M09 Research + Design slice is fully consumed; M05–M09 implementations are Lead-reviewed and merged, including LAB-REQ-02**. The complete **M10–M12 S4 Research + Design slice is now Lead-accepted**. The next bounded production step is **M10 learner-facing implementation** under the accepted Design, followed by independent Lead review. M11/LAB-REQ-01 and M12/EXP-03 remain later bounded implementation batches; do not combine them into the M10 packet.

## Current priority

1. Implement the bounded **M10 Networking I: IP, DNS & Transport learner packet** (L10-01–L10-03) from the accepted M10–M12 Design. Keep all network activity course-owned/loopback by default; use OS-assigned ports, deterministic refusal/read-timeout fixtures, capability-gated DNS observation, and no fixed errno/timing/recv partition assumptions. Do not implement M11/LAB-REQ-01 or M12/EXP-03 in this batch.

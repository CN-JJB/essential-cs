...
Learner Validation is deferred during authoring but remains mandatory before v1.0 / `RELEASED`. The accepted Research + Design slice for **M00–M04 is fully implemented through M04 after Lead review**. The **M05–M09 Research + Design slice is fully consumed; M05–M09 implementations are Lead-reviewed and merged, including LAB-REQ-02**. The complete **M10–M12 S4 Research + Design slice is now fully consumed; M10–M12 learner-facing implementations are Lead-reviewed and merged, including LAB-REQ-01 and EXP-03**. The **M13–M15 Data & Concurrency Research dossier is now Lead-accepted / READY FOR DESIGN**. There is not yet an accepted M13–M15 Design dossier, so the next bounded production step is **M13–M15 Data & Concurrency Design**, followed only then by bounded implementation passes. Do not dispatch M13 implementation before Design is Lead-accepted.

## Current priority

1. Produce the bounded **M13–M15 Data & Concurrency Design Dossier** from the Lead-accepted Research #83/#84 and canonical Blueprint. Convert the Research's authority/currentness/safety findings into exact Lesson, Lab, evidence, visual, preflight, cleanup, and implementation gates without changing the DAG or Registry.
2. Carry forward the Research's non-blocking risks explicitly: EXP-02 Target 1 source-route drift; scheduler-sensitive lost-update manifestation; OQ-BP-006; OSTEP link-only rights; SQLite planner/timing/write/space variability.
3. Do not author M13 learner-facing implementation until the Design is independently Lead-reviewed and accepted.
4. Keep independent Verification/Lead Review on every bounded implementation batch even though learner validation is deferred.
5. Keep **Issue #34 OPEN / DEFERRED / NON-BLOCKING** until real learning begins; never fabricate learner evidence.
6. Preserve OQ-BP-001 and OQ-BP-003 as RFC-gated/non-blocking and OQ-BP-006 as OPEN; Noble/Python 3.12 remains unverified.
7. Preserve the M03 GDB verification limitation as explicit technical debt; do not silently convert the blocked debugger path into PASS.
8. Preserve M04 microbenchmark evidence as environment-specific; do not turn the observed ratio into a curriculum constant.
9. Complete real learner validation and disposition material findings before v1.0 / `RELEASED`.

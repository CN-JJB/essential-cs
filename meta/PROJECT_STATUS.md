...
Learner Validation is deferred during authoring but remains mandatory before v1.0 / `RELEASED`. The accepted Research + Design slice for **M00–M04 is fully implemented through M04 after Lead review**. The **M05–M09 Research + Design slice is fully consumed; M05–M09 implementations are Lead-reviewed and merged, including LAB-REQ-02**. The complete **M10–M12 S4 Research + Design slice is now fully consumed; M10–M12 learner-facing implementations are Lead-reviewed and merged, including LAB-REQ-01 and EXP-03**. There is no accepted M13–M15 Research/Design dossier yet, so the next bounded production step is **M13–M15 Data & Concurrency Research**, followed by separate Design and bounded implementation passes. Do not dispatch M13 implementation before that Research/Design sequence is Lead-accepted.

## Current priority

1. Produce the bounded **M13–M15 Data & Concurrency Research Dossier** from the canonical Blueprint/DAG, Lab/Source selection map, Concept Registry, Competency Matrix, and source policy. Preserve M13/M14/M15 first-home discipline, especially EC-CON-014 Consistency at M14/L14-02 and EC-CON-015 Concurrency at M15/L15-01; research LAB-REQ-04, LAB-REQ-05, LAB-REQ-03 and the accepted Source Expedition route(s) before Design. Do not author learner Lessons/Labs in the Research pass.
2. After Research Lead acceptance, run a separate M13–M15 Design pass before any learner-facing implementation.
3. Keep independent Verification/Lead Review on every bounded implementation batch even though learner validation is deferred.
4. Keep **Issue #34 OPEN / DEFERRED / NON-BLOCKING** until real learning begins; never fabricate learner evidence.
5. Preserve OQ-BP-001 and OQ-BP-003 as RFC-gated/non-blocking and OQ-BP-006 as OPEN; Noble/Python 3.12 remains unverified.
6. Preserve the M03 GDB verification limitation as explicit technical debt; do not silently convert the blocked debugger path into PASS.
7. Preserve M04 microbenchmark evidence as environment-specific; do not turn the observed ratio into a curriculum constant.
8. Complete real learner validation and disposition material findings before v1.0 / `RELEASED`.

...
| **M16** | `L16-02` | "How do I call a remote function safely?" | Trace, Judge, Correctness | `EC-CON-005 Interface`, `EC-CON-008 Invariant`, `EC-CON-003 Representation`, `EC-CON-007 Specification` |
| **M17** | `L17-01` | "How do I keep data safe across machines?" | Judge, Explain, Estimate | `EC-CON-016 Durability`, `EC-CON-006 Trade-off`, `EC-CON-011 Caching` |
...
| **M17** | `L17-03` | "How consistent is 'strong enough'?" | Judge, Explain, Diagnose | `EC-CON-014 Consistency`, `EC-CON-006 Trade-off`, `EC-CON-013 Isolation` |
...
| **M18** | `L18-02` | "Do I need a distributor?" | Judge, Explain, Correctness | `EC-CON-006 Trade-off`, `EC-CON-015 Concurrency`, `EC-CON-009 Correctness` |
| **M19** | `L19-01` | "What is a container?" | Explain, Observe, Trace | `EC-CON-013 Isolation`, `EC-CON-018 Process`, `EC-CON-002 Abstraction` |
...
7. **Rejected: Unbounded "Exactly-Once" Delivery.** Exactly-once is an end-to-end property within a bounded transactional scope, requiring idempotent consumer logic. Transport alone cannot guarantee it across arbitrary side effects.
...
### 2. Prerequisites
- Hard: `L10-02` (TCP connection, byte streams, timeouts), `L15-01` (Concurrency, interleaving, shared-state obligations).
- Soft: `M14` (Transaction context).
...
- *SPECIFICATION:* POSIX socket timeout semantics (`SO_RCVTIMEO`); TCP retransmission timeouts (RFC 6298).
- *IMPLEMENTATION:* Python `socket.settimeout()` handling; OS errno (`ETIMEDOUT`, `ECONNREFUSED`).
- *CURRENT PRACTICE:* Microservice HTTP/RPC client timeout defaults (e.g., 2s, 30s) configured per environment.
...
### 9. Worked Example
A client initiates a fund transfer of \$100. The server receives the request, debits Account A, credits Account B, and writes to disk. Right before sending the HTTP `200 OK`, a network switch drops the return route. The client's timer fires after 3 seconds, raising a `TimeoutException`. The client cannot distinguish this from the request being dropped before arriving at the server.

### 10. Bounded Hands-On / Observation
Run a two-process Python test where a client sends requests to a server over localhost. A course-owned application shim introduces a 2000ms delay on request #3. The client has a 500ms timeout configured. The learner observes the client raising `TimeoutError` while the server process console prints `[SERVER] Processed request #3 successfully at T+2000ms`.

### 11. Evidence to Record
- Timestamp of client request dispatch.
- Timestamp and error code of client timeout abort.
...
- **PASS:** Client records `TimeoutError` while server log proves completion of the identical request ID.
...
- *Expected Observation:* The server prints success for both the original delayed request and the subsequent retry request.
...
Recheck RFC 9110 §9.2 status and POSIX socket timeout error specifications.
...
### 1. Purpose / Target Mental Model
Master the construction of resilient remote interfaces. Understand that remote procedure calls (RPC) must handle retries, avoid retry amplification storms using exponential backoff with full jitter, and prevent duplicate side effects using **Idempotency Keys** under a named operation contract.

### 2. Prerequisites
- Hard: `L16-01` (Partial failure, timeout ambiguity).
- Soft: `M14` (ACID transactions, unique constraints).
...
- `Correctness`: Prove that repeated execution of an idempotent RPC produces no additional intended side effects.

### 4. Canonical Concept First-Home vs. Revisit
- Canonical Revisits:
  - `EC-CON-005 Interface (接口)`: RPC interface contracts, parameter marshaling, and error boundaries.
  - `EC-CON-008 Invariant (不变量)`: Idempotency as a state-transition invariant: $f(f(x)) = f(x)$.
...
- Explain why a timeout cannot be safely retried without an idempotency contract.
- Implement exponential backoff with full jitter to decorrelate concurrent retries:
  $$t_{\text{sleep}} = \text{random}(0, \min(t_{\text{max}}, t_{\text{base}} \times 2^{\text{attempt}}))$$
- Explain why retry budgets and downstream cancellation context are mandatory in deep call graphs.
- Design an idempotency-key deduplication store with an explicit retention/eviction boundary.

### 6. Stable Principle
A retry without an idempotency guarantee converts network ambiguity into data corruption. An idempotency key must be atomically recorded with the business state mutation.

### 7. Specification vs. Implementation vs. Current-Practice Boundaries
- *PRINCIPLE:* Birrell & Nelson (1984) RPC semantics; Saltzer et al. (1984) end-to-end arguments.
- *SPECIFICATION:* RFC 9110 HTTP method idempotency (`GET`, `PUT`, `DELETE` are idempotent; `POST` is not).
...
- *Misconception:* "An idempotency key protects state forever." (False: all deduplication tables have finite TTL / retention horizons; late retries past the horizon can re-execute).
...
### 10. Bounded Hands-On / Observation
A Python script implements a counter service with an artificial delay. The client sends a payment increment with a 300ms timeout and an automatic retry loop (up to 3 retries).
- *Step 1 (Unsafe):* Run without idempotency keys. Observe counter equals 3 instead of 1 (duplicate execution).
- *Step 2 (Safe):* Attach an `idempotency_key` and enable atomic deduplication in SQLite. Observe counter equals 1 despite 3 client attempts.

### 11. Evidence to Record
- Table of attempts: attempt number, calculated backoff delay (ms), injected delay (ms), client outcome, server counter value.
- Proof of exact-once side effect on the counter under safe execution.
...
- *Hint 2:* Use a `UNIQUE` constraint or transactional lock (`INSERT INTO idempotency_keys VALUES (...)`) before executing the business mutation.
- *Expected Observation:* The second concurrent request fails the unique constraint and waits for the first request to finish.
...
- **Architecture:**
  - `RPCServer`: Binds to `127.0.0.1` on an ephemeral OS-assigned port (`port=0`). Runs a worker thread pool.
  ...
  - `RPCClient`: Dispatches requests with configurable timeout, retry count, and backoff policy (`EXPONENTIAL_JITTER` vs. `DETERMINISTIC`).

### 8.2 Execution Safety & Clean Reset
- The server binds only to localhost (`127.0.0.1`).
- All threads are daemon threads to ensure immediate process termination on exit.
- A shutdown watchdog triggers after 10 seconds maximum execution time.
...
- **Stopping Point:** Implementing the `TCPReceiver::receive()` and `TCPReceiver::send()` methods up to passing the public Checkpoint 2 tests. No subsequent checkpoints (TCP sender, router) are required.
...
### 2. Prerequisites
- Hard: `L16-01` (Partial failure, network delay), `L14-01` (Database transactions, WAL model).
- Soft: `M09` (Durability guarantees, fsync).
...
- `Estimate`: Calculate quorum requirements and replica failure tolerances for an $N$-node cluster.
...
### 6. Stable Principle
Replication is not automatic durability. Asynchronous replication trades durability for latency; synchronous replication trades availability for durability. Quorum overlap proves set intersection, not data freshness.
...
- *SPECIFICATION:* MySQL binlog replication protocols; Postgres streaming replication contracts (`synchronous_commit`).
- *IMPLEMENTATION:* Semi-sync replication plugins; Dynamo-style client-driven read-repair.
- *CURRENT PRACTICE:* Cloud managed database replication options (e.g., AWS RDS Multi-AZ vs. Read Replicas).
...
### 9. Worked Example
A 3-node cluster ($N=3$) configured with $W=2, R=2$. Node 1 and Node 2 accept a write for Key $K=V_2$ with timestamp $T_2$. Node 3 remains on $K=V_1$ with timestamp $T_1$. A reader queries Nodes 2 and 3 ($R=2$). The returned set contains $\{V_2, V_1\}$. Because the quorums overlap on Node 2, the client observes both values; only by inspecting the version metadata can the client correctly resolve $V_2$ as the latest value.
...
### 13. Progressive Support
- *Question:* In an $N=5$ system with $W=3, R=3$, how many node failures can the system tolerate while still serving reads and writes?
- *Hint 1:* What is the minimum number of nodes required to form a write quorum?
- *Hint 2:* If 2 nodes fail, are there still 3 nodes available? What if 3 nodes fail?
- *Expected Observation:* The system can tolerate $5 - 3 = 2$ node failures.
...
### 1. Purpose / Target Mental Model
Master the fundamental **Consensus Problem**: how a set of distributed processes decide on a single, irrevocable value despite message loss, arbitrary delays, and node crashes. Understand the danger of **split-brain** and explore how consensus protocols (Raft) guarantee safety under all conditions while relying on timing assumptions for liveness.

### 2. Prerequisites
- Hard: `L17-01` (Replication, quorums), `L16-01` (Network partitions, timeout ambiguity).
- Soft: `M14` (Write-Ahead Logging).

### 3. Primary Competencies
- `Explain`: Explain the split-brain hazard and how majority quorums ($> N/2$) prevent duplicate leaders.
...
- Explain how Raft achieves safety asynchronously while utilizing randomized timeouts to achieve liveness in partially synchronous periods.
...
### 6. Stable Principle
Consensus algorithms never sacrifice safety for liveness. Split-brain is prevented by requiring a strict majority ($> N/2$); two disjoint majorities cannot exist simultaneously in the same term.

### 7. Specification vs. Implementation vs. Current-Practice Boundaries
- *PRINCIPLE:* FLP Impossibility Theorem (1985); Paxos (Lamport 1998); Raft (Ongaro & Ousterhout 2014).
- *SPECIFICATION:* Raft Paper safety invariants (Election Safety, Leader Append-Only, Log Matching, Leader Completeness, State Machine Safety).
- *IMPLEMENTATION:* etcd Raft implementation in Go; HashiCorp Raft library.
- *CURRENT PRACTICE:* Production etcd / Kubernetes control plane clustering rules (minimum 3 or 5 nodes).
...
- *Misconception:* "A 3-node cluster is more available than a 2-node cluster for writes." (True: 3-node needs 2 alive ($f=1$); 2-node needs 2 alive ($f=0$); 2-node cluster has worse write fault-tolerance than 1-node!).
- *Distinction:* Safety (bad things never happen) vs. Liveness (good things eventually happen).

### 9. Worked Example
A 5-node cluster $\{A, B, C, D, E\}$. A network partition isolates $\{A, B\}$ from $\{C, D, E\}$. $A$ was the leader in Term 1. In the minority partition, $A$ cannot achieve a majority write quorum ($2 < 3$) and cannot commit new entries. In the majority partition, $C$ times out, increments term to 2, requests votes from $D$ and $E$, wins the election with 3 votes, and becomes leader. Split-brain is impossible because $\{A, B\}$ cannot win an election.
...
- Verify that a leader cannot be elected without the vote of at least one node containing all committed entries.

### 11. Evidence to Record
- Vote disposition table for all candidates.
- Formal proof statement identifying the overlapping voter node between the election quorum and prior commit quorum.

### 12. PASS / BLOCKED / NOT RUN Conditions
- **PASS:** 100% correct evaluation of Raft voting rules and leader completeness invariants.
...
- Title: "Consensus Safety vs. Liveness: Quorum Disjointness"
...
### 15. Stopping Point
Stop when the learner demonstrates that split-brain is mathematically prevented by majority overlap.
...
### 2. Prerequisites
- Hard: `L17-01` (Replication lag, quorums), `L14-02` (Database isolation levels, anomalies).
- Soft: `M12` (Web client state).
...
- `Diagnose`: Identify linearizability and monotonic read violations from multi-client execution traces.
...
### 6. Stable Principle
"Consistent" is meaningless without a named qualifier. Linearizability enforces global real-time ordering across all observers. When physical networks partition, zero latency guarantees and 100% availability cannot coexist with linearizability.

### 7. Specification vs. Implementation vs. Current-Practice Boundaries
- *PRINCIPLE:* Herlihy & Wing (1990) Linearizability; Gilbert & Lynch (2002) CAP theorem formal proof.
- *SPECIFICATION:* Jepsen verification consistency models; ANSI SQL isolation vs. distributed consistency.
...
### 8. Required Distinctions / Misconceptions
- *Misconception:* "Eventual consistency means data converges in a few milliseconds." (False: under persistent partitions or bugs, eventual consistency can take hours or require manual reconciliation).
...
### 9. Worked Example
Client $A$ writes $X = 1$ at $T=10$ and receives success at $T=15$. Client $B$ invokes a read on $X$ at $T=20$ and receives $X = 0$. In a linearizable system, this execution trace is invalid because Client $B$'s invocation occurred strictly after Client $A$'s response had completed. In an eventually consistent system without monotonic read guarantees, this trace is valid and represents an observable stale read anomaly.

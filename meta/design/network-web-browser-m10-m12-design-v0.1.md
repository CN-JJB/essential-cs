...
2. **Before / After Capability:**
   ...
   - *After:* Can explain how TLS 1.3 (RFC 9846) establishes an authenticated encrypted channel; distinguish PKIX certificate-path validation (RFC 5280) from application service-identity matching (RFC 9525); articulate forward-secrecy boundaries; and describe the metadata-visibility limits of ordinary ClientHello SNI and Encrypted Client Hello (RFC 9849).
3. **Prerequisites & Hidden-Prerequisite Support:**
   - Hard prerequisite: `L10-02` (TCP connections and streams).
   - Support: Dedicated course-owned certificate fixture and isolated verification context. Course maintenance provides a test root CA plus leaf key/certificate whose SAN covers the exact local service identity used by the activity. The learner trusts only the course CA in a dedicated context; no system trust store is modified. Implementation must record the fixture validity window and provide a maintenance/regeneration procedure so certificate expiry never becomes fabricated learner failure.
4. **Concepts:** Revisits **Trust Boundary** (EC-CON-017) and **Interface** (EC-CON-005). No new first homes.
5. **Mental Model:** Cryptographic channel plus identity verification under a trust policy. TLS negotiates traffic keys and authenticates the peer according to the selected handshake. In the certificate case, certificate-path validation and service-identity matching are separate checks. Success establishes the configured service identity under the client's trust policy; it does not establish business legitimacy, authorization, or application correctness.
6. **Mechanism Sequence:**
   ...
   $\text{Client validates certificate path (RFC 5280) + service identity (RFC 9525)} \xrightarrow{} \text{Client Finished} \xrightarrow{} \text{Encrypted Application Data (AEAD)}$
...
8. **Hands-on Progression (Observe / Build / Break / Explain / Judge):**
   - *Observe:* Run a local TLS server on `127.0.0.1` (port 0) using the course test certificate and dedicated CA. Connect with the actual Python TLS implementation configured with that CA and the matching service identity. Preflight must confirm TLS 1.3 capability before this case is treated as Core evidence; record the actual negotiated version/cipher rather than assuming a backend/version from the OS name.
   - *Break (Case 1: Service-Identity Mismatch):* Connect to the same loopback endpoint while asking the client to verify a deliberately non-matching reference identity (for example `mismatch.invalid`). Record the actual verification-failure disposition; do not machine-assert one exception message or platform-specific error string.
   - *Break (Case 2: Untrusted Anchor):* Connect using a dedicated verification context that does not trust the course test CA. Record the actual certificate/path verification failure for the named runtime; do not depend on the host's default trust store contents or one exact exception string.
...
14. **Progressive Support:**
    ...
    - *Expected Observation:* The trusted-course-CA case succeeds; the non-matching service-identity and untrusted-anchor cases fail verification. Record the actual exception classes/messages from the named runtime rather than requiring one fixed error text.

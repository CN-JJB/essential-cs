...
**Proposed size: 18 provisional concepts.** The first population includes the 15 initial Big Ideas already named by the canonical Registry as concepts, plus three additional candidate concepts that recur across several modules and are at high risk of definition drift: `Process`, `Durability`, and `Trust Boundary`. This proposal does **not** promote those three to new Big Ideas; Big Idea changes require separate architecture review. It deliberately excludes product names, commands, individual protocols, framework terms, and every noun in a Lesson.

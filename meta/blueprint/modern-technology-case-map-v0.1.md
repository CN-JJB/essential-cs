...
对于 **RFC-gated Open Question** 行，`REJECT / NOT CORE` 只表示“**当前 Blueprint 未纳入**”，不等于永久架构否决；只有正式 RFC/Decision 才能把该问题定案。
...
| HTTP/2 | 同一 HTTP semantics 下减少 HTTP/1.x 多连接与**应用层** head-of-line blocking 的部分成本；**TCP-level HOL 仍存在** | multiplexed streams、header compression、flow control；semantics 与 transport expression 分离 | M11/M12 | Modern protocol comparison | STABLE CORE MECHANISM | STABLE | 展示“同一语义，不同 transport expression”的演化，并为 HTTP/3/QUIC 的 transport-level 对照建立边界 | 不要求调优具体浏览器/server H2 stack；不得声称 HTTP/2 消除了 TCP HOL | 没有并发请求/协议比较目标时 | RFC 9113 + DevTools/trace 对照；避免把一次 benchmark 当原因 | 18–24 个月或规范重大更新 |
...
| Browser multi-process / site-isolation architecture (Chromium/Firefox cases) | 不可信内容、并行页面、渲染与多 subsystem 集成 | process isolation、event loop/render pipeline、origin/security boundaries | M12；EXP-03 | Stable browser mechanism with replaceable real-system cases | STABLE CORE MECHANISM | STABLE | 浏览器是跨 OS/network/runtime/security 的真实综合案例 | Chromium/Firefox 是可替换实现；具体 process model/site-isolation details 按 CURRENT implementation claim 处理；不教 browser product mastery | 若问题仅是 HTTP 或 JS 语法，避免引入完整 browser complexity | official architecture/source docs + DevTools/process observation | stable mechanism 18–24 个月；implementation details 6–12 个月 |
...
| LLM architecture / transformer theory / prompt engineering / AI app development | 快速变化的 AI 系统与应用方法 | —；**不由本 map 决定 stable Core principle** | None; OQ-BP-001 | **Not admitted in current Blueprint; pending RFC/Decision** | REJECT / NOT CORE | FRONTIER | OQ-BP-001 未决；这里是当前 non-admission，不是永久拒绝；本 map 无权把它们静默纳入 Core | 不得通过 technology case 侧门建立 AI Core track | 除未来 RFC 明确改变 scope 外，不进入 Core | 若研究 OQ，只用 current primary/paper evidence；本 v0.1 无 assessment claim | 3–6 个月仅用于 OQ/frontier reassessment |
| HCI / accessibility product or browser-feature training | human-facing feedback/accessibility/consent/recovery | —；existing P2/P9 evidence hooks 不等于 settled Core track | P2/P9 hooks only; OQ-BP-003 | **Not admitted in current Blueprint; pending RFC/Decision** | REJECT / NOT CORE | STABLE | OQ-BP-003 未决；这里是当前 non-admission，不是永久拒绝；browser case 不能静默扩张 HCI Core | 保留现有 evidence hooks；任何新增 canonical HCI content 需 RFC/Decision | 除 existing hooks 或未来 RFC 决定外，不扩展 | 若未来评估 OQ，使用 standards/primary accessibility evidence；本 map 不创建 rubric | 18–24 个月或 OQ 状态变化 |
...
| REJECT / NOT CORE | 2 | 两项均为**当前 Blueprint non-admission**：LLM/prompt/AI-app track 与 HCI/accessibility product track 不得由本 map 静默入 Core；OQ-BP-001/003 的最终 disposition 仍需 RFC/Decision |

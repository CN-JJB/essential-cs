...
| `10` Concurrency | Classic semaphore/producer-consumer or bounded-buffer exercise (candidate family; exact source to be selected in dossier) | Interleavings, blocking, invariant and progress reasoning. | `10` | **Adapt pending source selection**: use a proven course exercise, not a generated race demo. | Threads and shared state; medium. | POSIX/Python environment; low runtime; safe if bounded. | Candidate family not yet tied to one authoritative handout; provenance **OPEN**. | Supports P5; a project race harness may remain custom only if no candidate fits integration. |
...
4. **Concurrency-to-application integration:** Classic bounded-buffer exercises and database isolation materials teach mechanisms separately. A project-specific race harness may be needed for P5, but only after adopting/adapting one proven concurrency lab.

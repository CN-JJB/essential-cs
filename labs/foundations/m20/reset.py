#!/usr/bin/env python3
"""
reset.py — Fail-Closed, Idempotent Cleanup for M20 Runtime Artifacts
===================================================================

Removes course-owned scratch artifacts under labs/foundations/m20/.scratch/ including:
...
M20_DIR = os.path.dirname(os.path.abspath(__file__))
SCRATCH_DIR = os.path.join(M20_DIR, ".scratch")
PYCACHE_DIR = os.path.join(M20_DIR, "__pycache__")


def reset_m20_environment(verbose: bool = True) -> int:
    errors = []
    removed_count = 0
    ...
    if errors:
        message = "M20 cleanup incomplete: " + "; ".join(errors)
        if verbose:
            print(f"[RESET ERROR] {message}", file=sys.stderr)
        raise RuntimeError(message)

    if verbose:
        print("[RESET] M20 course-owned scratch is clean.")
    return removed_count


if __name__ == "__main__":
    try:
        reset_m20_environment(verbose=True)
        sys.exit(0)
    except RuntimeError:
        sys.exit(1)

#!/usr/bin/env python3
"""
Fail-closed, idempotent cleanup for M18 course-owned runtime artifacts.

Removes files created under labs/foundations/m18/.scratch/ including:
...
M18_DIR = os.path.dirname(os.path.abspath(__file__))
SCRATCH_DIR = os.path.join(M18_DIR, ".scratch")
PYCACHE_DIR = os.path.join(M18_DIR, "__pycache__")


def reset_m18_environment(verbose: bool = True) -> int:
    errors = []
    removed_count = 0
    ...
    if os.path.isdir(SCRATCH_DIR):
        ...
        # Also try removing scratch dir if empty
        try:
    ...
    if errors:
        message = "M18 cleanup incomplete: " + "; ".join(errors)
        if verbose:
            print(f"[RESET ERROR] {message}", file=sys.stderr)
        raise RuntimeError(message)

    if verbose:
        print("[RESET] M18 course-owned scratch is clean.")
    return removed_count


if __name__ == "__main__":
    try:
        reset_m18_environment(verbose=True)
        sys.exit(0)
    except RuntimeError:
        sys.exit(1)

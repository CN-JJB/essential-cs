#!/usr/bin/env python3
"""
reset.py — Fail-Closed, Idempotent Cleanup for M23 Runtime Artifacts
===================================================================

Removes course-owned scratch artifacts under labs/foundations/m23/.scratch/ including:
...
M23_DIR = os.path.dirname(os.path.abspath(__file__))
SCRATCH_DIR = os.path.join(M23_DIR, ".scratch")
PYCACHE_DIR = os.path.join(M23_DIR, "__pycache__")


def reset_m23_environment(verbose: bool = True) -> int:
    errors = []
    removed_count = 0
    ...
    if errors:
        message = "M23 cleanup incomplete: " + "; ".join(errors)
        if verbose:
            print(f"[RESET ERROR] {message}", file=sys.stderr)
        raise RuntimeError(message)

    if verbose:
        print("[RESET] M23 course-owned scratch is clean.")
    return removed_count


if __name__ == "__main__":
    try:
        reset_m23_environment(verbose=True)
        sys.exit(0)
    except RuntimeError:
        sys.exit(1)
